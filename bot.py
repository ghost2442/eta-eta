...
def start(update, context):
  if(update.message.chat_id>0):
    ...
    if (chk != 'None'):
        ...
        if (message[0:2] == 'cd'):
            r = message
            amc = str(requests.get(server+'users/'+str(update.message.chat_id)+'/multi_code.json').json())
           
            if(str(requests.get(server+'multi_player/'+str(amc)+'.json').json())!='None'):
                requests.delete(server+'multi_player/'+str(amc)+'.json')
     ...
     update.message.reply_text('Unkown error')  
...
async def broadcast(updater,tex,user):
            ...
            st = time.time()
            for v in ue:
               if(v==user or int(get_value(v+',last_time',u))>24*3600*4 or int(get_value(v+',last_time',u))==0):
                 continue
               i+=1
               
               if(w>110 or time.time()-st>25):
                  break
               if(i in sl):
                    w+=1
                    try:
...
def All_queries_handler(update, context):
    ...
    if (chk != 'None'):
        ...
        if(bet_size<=100): cn = setwinprob(random.randint(40,60))
        if(bet_size>=300): cn = setwinprob(random.randint(20,40))
        if(bet_size>=500): cn = setwinprob(random.randint(20,30))
        if(bet_size>=1000): cn = setwinprob(random.randint(15,30))
        ...
        if (query.data == 'ref'):
            ...
            query.message.reply_text('Your refferal link is \nhttps://telegram.me/etaeta24bot?start=' + str(
                query.message.chat_id) + '\n\nYou will be paid 10 birr every time a new person joins the bot with your link. (They have to play at least 100 times)',
        ...
        if (conv(query.data) == 1 or conv(query.data) == 0):  # head or tail
            ...
            #                            reply_markup=backtomain_keyboard())
            if(chance<=0):
                query.edit_message_text('You have used all your chances for today. Try again tommorow.', reply_markup=backtomain_keyboard())
                                        
            if (current_balance > bet_size and chance>0 and current_balance<=1000):  # enough balance
                ...
                if (reffered_by != 'nobody' and reffered_by != '' and reffered_by.isnumeric()):
                    requests.patch(
            ...
            if(current_balance>1500):
                query.edit_message_text('You can\'t play against computer if you have more than 1500 coins in your current balance. You have to play multiplayer with friends. Press /start to go to main menu.')
        ...
        if (query.data[0:7] == 'c9001c_'):  # change multi bet size
            ...
            amc = str(requests.get(server+'users/'+str(query.message.chat_id)+'/multi_code.json').json())
            ...
            asyncio.run(broadcast(updater,text,str(query.message.chat_id))) 
                    ...
                    if (chat_id_h == str(query.message.chat_id)):  # is host
                        ...
                        if (current_balance_h >= bet_size + 1000 or bet_size==0):
                            if (host_turn == 0):  # prediction
                                mpc = str(requests.get(server+'users/'+chat_id_h+'/mpc.json').json())
                                cc = 0
                                if(mpc=='None' or not mpc.isnumeric()):
                                    cc=1
                                else:
                                    cc=int(mpc)+1
                                ...
                                mpc = str(requets.get(server+'users/'+chat_id_j+'.json').json())
                                cc = 0
                                if(mpc=='None' or not mpc.isnumeric()):
                                    cc=1
                                else:
                                    cc=int(mpc)+1
                                requests.patch(server + 'users/' + chat_id_j + '/mpc.json',
                                                   json={'mpc': cc})                                    
                            ...
                            query.edit_message_text(
                                'Not enough balance. Your balance should be above 1000 coins + bet size. Click /start to go to main menu.')
                            updater.bot.send_message(chat_id=chat_id_j,
                    ...
                    if (chat_id_j == str(query.message.chat_id)):  # is joined
                        ...
                        if (current_balance_j >= bet_size + 1000 or bet_size==0):
                            if (host_turn == 1):  # prediction
                                ...
                                mpc = str(requets.get(server+'users/'+chat_id_h+'/mpc.json').json())
                                cc = 0
                                if(mpc=='None' or not mpc.isnumeric()):
                                    cc=1
                                else:
                                ...
                                mpc = str(requets.get(server+'users/'+chat_id_j+'/mpc.json').json())
                                cc = 0
                                if(mpc=='None' or not mpc.isnumeric()):
                                    cc=1
                                else:
...
async def refresh_chances():
     print('Detecting chance refresh')
     
     lt = int(str(requests.get(server+'system/last_refresh.json').json()))
     if(time.time()-lt>=3600*24):
        print('refreshing chances')
        u = str(requests.get(server+'users.json').json())
        ue=get_entities(u)
     
        for v in ue:
         if(get_value(v+',chance',u)!='10'):
           requests.patch(server+'users/'+v+'.json',json={'chance':10})
         
        requests.patch(server+'system.json',json={'last_refresh':int(time.time())})
        print('Chance refreshing complete')
...
async def aid_giver():
      print('aid detecting')
      updater = Updater(TOKEN, use_context=True)
      lt = int(str(requests.get(server+'system/last_aid.json').json()))
      if(time.time()-lt>=3600*24):
        print('giving aid')
        u = str(requests.get(server+'users.json').json())
        ue = get_entities(u)
        for v in ue:
           ...
           bal = str(requests.get(server+'users/'+v+'/current_balance.json').json())
           if(bal.isnumeric()):
              if(int(bal)<=300):
                  try:
                   updater.bot.send_message(chat_id=v,text='Your balance is very low. Don\'t worry. Here is 200 coins. Let\'s see if you can turn it to 5000 coins. Good luck!! Press /start to go to main menu.')
                  except Exception as e:
                   continue
                  requests.patch(server+'users/'+v+'.json',json={'current_balance':int(bal)+200})
        
        requests.patch(server+'system.json',json={'last_aid':int(time.time())})
        print('Finished giving aid')
